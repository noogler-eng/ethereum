...
// this is in read mode
const contractInstance = new ethers.Contract(LinkTokenAddress, abi, provider);

const main = async () => {
  try {
    let balanceOfSender = await contractInstance.balanceOf(sender);
    let balanceOfReceipnt = await contractInstance.balanceOf(recepient);
    console.log(
      "balance of sender before: ",
      ethers.formatEther(balanceOfSender)
    );
    console.log(
      "balance of recepient before: ",
      ethers.formatEther(balanceOfReceipnt)
    );

    // first wallet needs to be connected with the contract and then the new instance should be used to write in contract
    const contractwithWallet: BaseContract = contractInstance.connect(wallet);

    // sedning the transaction
    // @ts-ignore
    const tx: TransactionResponse = await contractwithWallet.transfer(
      recepient,
      ethers.parseEther("5.05")
    );
    tx.wait();
    console.log(tx.toJSON());

    balanceOfSender = await contractInstance.balanceOf(sender);
    balanceOfReceipnt = await contractInstance.balanceOf(recepient);
    console.log(
      "balance of sender before: ",
      ethers.formatEther(balanceOfSender)
    );
